...
const Tooltip = React.forwardRef(({ item, event, itemId }, ref) => {
    ...
    useEffect(() => {
        ...
        if (event.type === "mouseover" && item.id === itemId) {
            handleItemMouseOver(event, tooltipRef);
            setIsVisible(true);
        }
    }, [event]);
    ...
    const hiddenStyle = isVisible ? { top: tooltipPosition.y, left: tooltipPosition.x } : { visibility: 'hidden', position: 'absolute', top: '0', left: '0' };

    return (
        <>
            <div ref={tooltipRef} style={{ ...tooltipStyle, ...hiddenStyle }}>
